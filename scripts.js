document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        if (!hamburgerIcon.contains(event.target) && !dropdown.contains(event.target)) {
        ...
        cart.forEach((item, index) => {
            ...
            cartItem.innerHTML = `
                <div class="product-item">
                    <img src="path-to-image/${item.name.replace(/\s+/g, '-').toLowerCase()}.jpg" alt="${item.name}">
        ...
        if (cart.length > 0) {
            ...
            cart.forEach((item, index) => {
                ...
                productDetail.innerHTML = `
                    <h3>${item.name} (${item.category || 'Unknown Category'})</h3>
                    <input type="hidden" name="product-name-${index}" value="${item.name}">
                    <input type="hidden" name="product-category-${index}" value="${item.category || 'Unknown Category'}">
    ...
    orderForm.addEventListener('submit', (e) => {
        ...
        cart.forEach((item, index) => {
            orderSummary += `\nProduct: ${orderDetails[`product-name-${index}`]} (${orderDetails[`product-category-${index}`]})\nQuantity: ${orderDetails[`quantity-${index}`]}`;
        });
    ...
    filterForm.addEventListener('submit', (e) => {
        ...
        if (gender) {
            products.forEach(product => {
                if (!product.classList.contains(gender)) {
        ...
        if (price) {
            products = Array.from(products).filter(product => product.style.display !== 'none');
            products.sort((a, b) => {
